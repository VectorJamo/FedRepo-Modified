...
class ScaffoldRS():
    ...
    def train(self):
        ...
        for r in range(1, self.args.max_round + 1):
            ...
            for client in sam_clients:
                # control to gpu
                self.set_control_cuda(self.client_controls[client], False) # ** argument to use CUDA set from True to False ** 
    ...
    def update_local(
        ...
        for t in range(n_total_bs + 1):
            ...
            model.train()
            try:
                batch_x, batch_y = next(loader_iter) # ** Code was modified here **
            except Exception:
                loader_iter = iter(train_loader)
                batch_x, batch_y = next(loader_iter) # ** Code was modified here **
